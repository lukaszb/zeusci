...
zeus.controller 'ProjectBreadcrumbsController', ($scope, $state, $stateParams) ->
    ...
    getBreadcrumbs = () ->
        ...
        if $state.current.name == 'buildset_create'
            breadcrumbs.push {name: "Force a build", url: $state.url}
...
zeus.controller 'BuildsetCreateController', ($scope, $routeParams, Buildset) ->

    $scope.initController = ->
        $scope.buildset = {}
        $scope.controlState = {processing: false}

    $scope.createBuildset = ->
        $scope.controlState.processing = true
        buildset = new Buildset($scope.buildset)
        buildset.$post {name: $scope.project.name}, (bs) ->
            $scope.controlState.processing = false
